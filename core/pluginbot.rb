...
#
#echo "Dies ist ein Test" | ./piper --model de_DE-thorsten-medium.onnx --output_raw | sox -V -t raw -b 16 -e signed -c 1 -r 22050 - -t raw -b 16 -e signed -c 1 -r 48k - | aplay -r 48000 -f S16_LE -t raw -
#
...
class Mumble
  ...
  def mumble_start
    ...
    if @cli.connected?
      logger "OK: Bot successfully connected to \"#{Conf.gvalue("mumble:host")}:#{Conf.gvalue("mumble:port")}\"."
      begin
        @cli.join_channel(Conf.gvalue("mumble:channel"))
        logger "OK: Bot entered configured channel \"#{Conf.gvalue("mumble:channel")}\"."
      rescue
      ...
      begin
        @cli.set_comment('MR-Bot Thorsten (sorry not functional yet)')
...
begin
  puts "OK: Bot is running."
  client.mumble_start
rescue
  puts "ERROR: Bot could not start. (#{$!})"
  puts $!
  puts $@
end
